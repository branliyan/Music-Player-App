...
public class Song extends AudioContent implements Comparable<Song> // implement the Comparable interface
{
	...
	public void play()
	{
		super.setAudioFile(getLyrics()); // this will set the audio file to the lyrics
		super.play(); // will play the audio file that was just set above
	}
	...
	{
		Song otherSong = (Song) other; // casts the unpointed object reference to that of a song
		return (super.equals(otherSong) && getComposer().equals(otherSong.getComposer()) && getArtist().equals(otherSong.getArtist()));
	}
